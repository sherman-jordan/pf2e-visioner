...
import {
  discoverDiversionObservers,
  previewDiversionResults,
} from "./create-a-diversion-logic.js";
...
function extractActionData(message) {
  ...
  const isDamageRoll =
    ...
    (firstRoll &&
      (firstRoll.isDamage === true ||
        (typeof DamageRoll !== "undefined" &&
          firstRoll instanceof DamageRoll) ||
        (typeof CONFIG?.Dice?.DamageRoll !== "undefined" &&
          firstRoll instanceof CONFIG.Dice.DamageRoll) ||
        (typeof firstRoll?.options?.type === "string" &&
          firstRoll.options.type.includes("damage")))) ||
  ...
  if (!isHiddenOrUndetectedToken && context?.options) {
    ...
    if (!isHiddenOrUndetectedToken && origin?.rollOptions) {
      isHiddenOrUndetectedToken = origin.rollOptions.some(
        (option) =>
          option.includes("effect:hidden-from") ||
          option.includes("effect:undetected-from") ||
          option.includes("hidden-from") ||
  ...
  if (
    (isSeekAction ||
      ...
      isSneakAction ||
      isCreateADiversionAction) &&
    (!message.rolls?.length || !actorToken)
  )
    return null;
  ...
  if (isConsequencesAction) {
    ...
    const indicatesUndetected = [...contextOptions, ...originOptions].some(
      (opt) =>
    ...
    const indicatesHidden =
      !indicatesUndetected &&
      [...contextOptions, ...originOptions].some(
        (opt) =>
    ...
    try {
      ...
      if (itemConds.some((c) => c?.slug === "undetected"))
        attackerConditionIndicator = "undetected";
      else if (itemConds.some((c) => c?.slug === "hidden"))
        attackerConditionIndicator = "hidden";
      else {
        const legacyConds = actorToken.actor?.conditions?.conditions || [];
        if (legacyConds.some((c) => c?.slug === "undetected"))
          attackerConditionIndicator = "undetected";
        else if (legacyConds.some((c) => c?.slug === "hidden"))
          attackerConditionIndicator = "hidden";
      }
    } catch (_) {}
  ...
  try {
    ...
    if (!hasValidTargets) {
      ...
      const pendingForGM =
        (actionData.actionType === "seek" &&
          game.user.isGM &&
          !!message.flags?.["pf2e-visioner"]?.seekTemplate?.hasTargets) ||
        (actionData.actionType === "point-out" &&
          game.user.isGM &&
          !!message.flags?.["pf2e-visioner"]?.pointOut?.hasTargets);
      // Always allow GM to see Point Out actions even if pre-check cannot confirm targets
      const allowPointOutForGM =
        actionData.actionType === "point-out" && game.user.isGM;
      // For consequences, hide button when no valid targets, otherwise show as normal
      const allowConsequencesForGM =
        actionData.actionType === "consequences" ? false : false;
      if (
        !allowSeekForGM &&
        !allowSeekForPlayer &&
        !pendingForGM &&
        !allowPointOutForGM &&
        !allowConsequencesForGM
      ) {
...
function checkConsequencesTargets(actionData, potentialTargets) {
  ...
  for (const target of potentialTargets) {
    ...
    if (enforceRAW && shouldFilterAlly(actionData.actor, target, "enemies"))
      continue;
    ...
    try {
      const itemTypeConditions =
        actionData.actor?.actor?.itemTypes?.condition || [];
      const legacyConditions =
        actionData.actor?.actor?.conditions?.conditions || [];
...
function checkSeekTargets(actionData, potentialTargets) {
  ...
  for (const target of potentialTargets) {
    ...
    if (target.actor) {
      ...
      const isHiddenOrUndetected = conditions.some(
        (c) =>
          c.slug === "hidden" ||
          c.slug === "undetected" ||
          c.slug === "concealed",
...
function buildAutomationPanel(actionData, message) {
  ...
  const msgForPanel = isSeek
    ? message || game.messages.get(actionData.messageId)
    : null;
  const hasPendingTemplateFromPlayer =
    isSeek &&
    !!msgForPanel?.flags?.["pf2e-visioner"]?.seekTemplate &&
    game.user.isGM;
  const pendingHasTargets =
    !!msgForPanel?.flags?.["pf2e-visioner"]?.seekTemplate?.hasTargets;
  const hasPendingPointOutFromPlayer =
    isPointOut &&
    !!msgForPanel?.flags?.["pf2e-visioner"]?.pointOut &&
    game.user.isGM;
  const pendingPointOutHasTargets =
    !!msgForPanel?.flags?.["pf2e-visioner"]?.pointOut?.hasTargets;
...
function bindAutomationEvents(panel, message, actionData) {
  ...
  panel.on("click", "[data-action]", async (event) => {
    ...
    try {
      ...
      if (
        action === "setup-seek-template" &&
        actionData.actionType === "seek"
      ) {
        await setupSeekTemplate(actionData);
      } else if (
        action === "remove-seek-template" &&
        actionData.actionType === "seek"
      ) {
          ...
          if (typeof pending.rollTotal === "number") {
            actionData.roll = {
              ...
              dice: [
                {
                  total:
                    typeof pending.dieResult === "number"
                      ? pending.dieResult
                      : undefined,
                },
              ],
        ...
        if (game.user.isGM) {
          try {
            ...
            if (modulePointOut?.targetTokenId) {
              ...
              actionData.context.target = {
                token: modulePointOut.targetTokenId,
              };
            } else if (msg?.flags?.pf2e?.target?.token) {
              actionData.context = actionData.context || {};
              actionData.context.target = {
                token: msg.flags.pf2e.target.token,
              };
            } else if (!actionData.context?.target?.token) {
              // As a last resort, resolve based on the pointer's best target
              const pointerId =
                modulePointOut?.pointerTokenId || actionData.actor?.id;
              const pointerToken = pointerId
                ? canvas.tokens.get(pointerId)
                : null;
              if (pointerToken) {
                const { findBestPointOutTarget } = await import(
                  "./point-out-logic.js"
                );
            ...
            try {
              if (!modulePointOut) {
                ...
                if (targetId) {
                  ...
                  if (tok) {
                    const point = tok.center || {
                      x: tok.x + (tok.w ?? tok.width * canvas.grid.size) / 2,
                      y: tok.y + (tok.h ?? tok.height * canvas.grid.size) / 2,
                    };
        ...
        (action === "open-sneak-results" &&
          actionData.actionType === "sneak") ||
        (action === "open-diversion-results" &&
          actionData.actionType === "create-a-diversion") ||
        (action === "open-consequences-results" &&
          actionData.actionType === "consequences")
      ) {
        // Directly delegate to unified preview handler for these actions
        await previewActionResults(actionData);
      } else if (
        action === "apply-now-seek" &&
        actionData.actionType === "seek"
      ) {
        await applyNowSeek(actionData, button);
      } else if (
        action === "apply-now-point-out" &&
        actionData.actionType === "point-out"
      ) {
        ...
        action === "revert-now-seek" &&
        actionData.actionType === "seek"
      ) {
        await revertNowSeek(actionData, button);
      } else if (
        action === "revert-now-point-out" &&
        actionData.actionType === "point-out"
      ) {
  ...
  if (actionData.actionType === "seek") {
    ...
    if (game.user.isGM) {
      ...
      try {
        ...
        let hasTarget =
          actionData.context?.target?.actor ||
...
async function applyNowSeek(actionData, button) {
  try {
    const { discoverSeekTargets, analyzeSeekOutcome } = await import(
      "./seek-logic.js"
    );
    ...
    await applyVisibilityChanges(actionData.actor, changes, {
      direction: "observer_to_target",
    });
    // Cache original states for revert
    try {
      appliedSeekChangesByMessage.set(
        actionData.messageId,
        changes.map((c) => ({
    ...
    button
      ...
      .attr("data-action", "revert-now-seek");
  } catch (e) {
    console.error(e);
  }
}
...
async function applyNowPointOut(actionData, button) {
  try {
    const {
      analyzePointOutOutcome,
      discoverPointOutAllies,
      getPointOutTarget,
      findBestPointOutTarget,
    } = await import("./point-out-logic.js");
    ...
    if (!target) {
      ui.notifications.info(`${MODULE_TITLE}: No target for Point Out`);
      return;
    }
    ...
    try {
      if (game.user.isGM && target) {
        const point = target.center || {
          x: target.x + (target.w ?? target.width * canvas.grid.size) / 2,
          y: target.y + (target.h ?? target.height * canvas.grid.size) / 2,
        };
        if (typeof canvas.ping === "function") {
          canvas.ping(point, {
            color: game.user?.color,
    ...
    // Cache for revert
    try {
      appliedPointOutChangesByMessage.set(
        actionData.messageId,
        changes.map((ch) => ({
          allyId: ch.target.id,
    ...
    button
      .html('<i class="fas fa-undo"></i> Revert Changes')
      .attr("data-action", "revert-now-point-out");
  } catch (e) {
    console.error(e);
  }
}
...
async function applyNowGeneric(actionData, button) {
  try {
    ...
    if (actionData.actionType === "hide") {
      const { discoverHideObservers, analyzeHideOutcome } = await import(
        "./hide-logic.js"
      );
      const observers = discoverHideObservers(actionData.actor, false, false);
      ...
      try {
        appliedHideChangesByMessage.set(
          actionData.messageId,
          changes.map((o) => ({
      ...
      button
        .html('<i class="fas fa-undo"></i> Revert Changes')
        .attr("data-action", "revert-now-generic");
      return;
    }
    // Sneak: same directionality as Hide
    if (actionData.actionType === "sneak") {
      const { discoverSneakObservers, analyzeSneakOutcome } = await import(
        "./sneak-logic.js"
      );
      ...
      try {
        appliedSneakChangesByMessage.set(
          actionData.messageId,
          changes.map((o) => ({
    ...
    if (actionData.actionType === "create-a-diversion") {
      const { discoverDiversionObservers, analyzeDiversionOutcome } =
      ...
      try {
        appliedDiversionChangesByMessage.set(
          actionData.messageId,
          changes.map((o) => ({
    ...
    if (actionData.actionType === "consequences") {
      ...
      const { findPotentialTargets, computeVisibilityBetween } = await import(
        "./consequences-logic.js"
      );
      ...
      try {
        appliedConsequencesChangesByMessage.set(
          actionData.messageId,
          changed.map((o) => ({
...
async function revertNowSeek(actionData, button) {
  try {
    ...
    if (!cached.length) {
      ui.notifications.info(`${MODULE_TITLE}: Nothing to revert`);
      return;
    }
    const changes = cached
      .map((entry) => {
        const tok =
          canvas?.tokens?.get?.(entry.targetId) ||
          canvas.tokens.placeables.find((t) => t.id === entry.targetId);
        if (!tok) return null;
        return {
          target: tok,
          newVisibility: entry.oldVisibility,
          changed: true,
        };
      })
      .filter(Boolean);
    ...
    try {
      appliedSeekChangesByMessage.delete(actionData.messageId);
    } catch (_) {}
    button
      .html('<i class="fas fa-check-double"></i> Apply Changes')
      .attr("data-action", "apply-now-seek");
  } catch (e) {
    console.error(e);
  }
}
...
async function revertNowPointOut(actionData, button) {
  try {
    ...
    const cached =
      appliedPointOutChangesByMessage.get(actionData.messageId) || [];
    let changes = cached
      .map((entry) => {
        const ally =
          canvas?.tokens?.get?.(entry.allyId) ||
          canvas.tokens.placeables.find((t) => t.id === entry.allyId);
        const target =
          canvas?.tokens?.get?.(entry.targetTokenId) ||
          canvas.tokens.placeables.find((t) => t.id === entry.targetTokenId);
    ...
    if (!changes.length) {
      ...
      if (!resolvedTarget) {
        ui.notifications.info(`${MODULE_TITLE}: No target`);
        return;
      }
    ...
    try {
      appliedPointOutChangesByMessage.delete(actionData.messageId);
    } catch (_) {}
    button
      .html('<i class="fas fa-check-double"></i> Apply Changes')
      .attr("data-action", "apply-now-point-out");
  } catch (e) {
    console.error(e);
  }
}
  ...
  try {
    ...
    if (actionData.actionType === "hide") {
      ...
      let changes = cached
        .map((entry) => {
          const observer =
            canvas?.tokens?.get?.(entry.observerId) ||
            canvas.tokens.placeables.find((t) => t.id === entry.observerId);
      ...
      try {
        appliedHideChangesByMessage.delete(actionData.messageId);
      } catch (_) {}
      button
        .html('<i class="fas fa-check-double"></i> Apply Changes')
        .attr("data-action", "apply-now-generic");
      return;
    }
    ...
    if (actionData.actionType === "sneak") {
      ...
      try {
        appliedSneakChangesByMessage.delete(actionData.messageId);
      } catch (_) {}
      button
        .html('<i class="fas fa-check-double"></i> Apply Changes')
        .attr("data-action", "apply-now-generic");
    ...
    if (actionData.actionType === "create-a-diversion") {
      const cached =
        appliedDiversionChangesByMessage.get(actionData.messageId) || [];
      let changes = cached
        .map((entry) => {
          const observer =
            canvas?.tokens?.get?.(entry.observerId) ||
      ...
      if (!changes.length) {
        ...
        changes = outcomes
          .filter((o) => o.changed)
          .map((o) => ({
            target: o.observer || o.token || o.target,
      ...
      try {
        appliedDiversionChangesByMessage.delete(actionData.messageId);
      } catch (_) {}
      button
        .html('<i class="fas fa-check-double"></i> Apply Changes')
        .attr("data-action", "apply-now-generic");
    ...
    if (actionData.actionType === "consequences") {
      const cached =
        appliedConsequencesChangesByMessage.get(actionData.messageId) || [];
      let changes = cached
        .map((entry) => {
          const observer =
            canvas?.tokens?.get?.(entry.observerId) ||
      ...
      try {
        appliedConsequencesChangesByMessage.delete(actionData.messageId);
      } catch (_) {}
      button
        .html('<i class="fas fa-check-double"></i> Apply Changes')
        .attr("data-action", "apply-now-generic");
        ...
        const createHookId = Hooks.on("createMeasuredTemplate", async (doc) => {
          ...
          try {
            ...
            actionData.seekTemplateRadiusFeet =
              Number(doc.distance) || distance;
            const targets = discoverSeekTargets(
              actionData.actor,
              false,
              actionData.seekTemplateRadiusFeet,
          ...
          const pointerHandler = async (event) => {
            ...
            try {
              ...
              if (created) {
                ...
                actionData.seekTemplateRadiusFeet =
                  Number(created.distance) || distance;
                const targets = discoverSeekTargets(
                  actionData.actor,
                  false,
                  actionData.seekTemplateRadiusFeet,
    ...
    await new Promise((resolve) => {
      const createHookId = Hooks.on("createMeasuredTemplate", async (doc) => {
        ...
        try {
          ...
          const roll =
            actionData.roll ||
            game.messages.get(actionData.messageId)?.rolls?.[0] ||
            null;
          const rollTotal = roll?.total ?? null;
          const dieResult =
            roll?.dice?.[0]?.total ?? roll?.terms?.[0]?.total ?? null;
