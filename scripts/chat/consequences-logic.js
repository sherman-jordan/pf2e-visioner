  ...
  try {
    ...
    const outcomes = await Promise.all(
      potentialTargets.map(async (target) => {
        ...
        const currentVisibility = await computeVisibilityBetween(
          target,
          attackingToken,
        );
    ...
    let filteredOutcomes = enforceRAW
      ? outcomes.filter(
          (outcome) =>
            outcome.currentVisibility === "hidden" ||
    ...
    if (enforceRAW && filteredOutcomes.length === 0) {
      ui.notifications.info(
        `${MODULE_TITLE}: No valid Consequences targets found`,
      );
      return;
    }
    ...
    console.error(
      `${MODULE_TITLE}: Error preparing consequences preview:`,
      error,
    );
    ui.notifications.error(
      `${MODULE_TITLE}: Failed to prepare consequences preview`,
    );
  }
}
...
export function findPotentialTargets(attackingToken) {
  ...
  const results = allTokens.filter((token) => {
    ...
    if (enforceRAW && shouldFilterAlly(attackingToken, token, "enemies"))
      return false;
  ...
  const checkObserverEffects = (observer, target) => {
    ...
    if (actor.effects) {
      ...
      for (const effect of effects) {
        ...
        if (
          (effectName.includes("hidden from") ||
            effectName.includes("hidden")) &&
          (effectOrigin.includes(target.id) ||
            effectName.includes(target.name.toLowerCase()))
        ) {
          return "hidden";
        }

        if (
          (effectName.includes("undetected from") ||
            effectName.includes("undetected")) &&
          (effectOrigin.includes(target.id) ||
            effectName.includes(target.name.toLowerCase()))
        ) {
          return "undetected";
    ...
    if (actor.getRollOptions) {
      ...
      for (const option of rollOptions) {
        ...
        if (
          ...
          (option.includes(target.id) ||
            option.includes(target.name.toLowerCase()))
        ) {
          return "hidden";
        }
  ...
  const checkTargetEffects = (target, observer) => {
    ...
    if (actor.effects) {
      ...
      for (const effect of effects) {
        ...
        if (
          (effectName.includes("hidden to") ||
            effectName.includes("hidden from")) &&
          (effectOrigin.includes(observer.id) ||
            effectName.includes(observer.name.toLowerCase()))
        ) {
          return "hidden";
        }

        if (
          (effectName.includes("undetected to") ||
            effectName.includes("undetected from")) &&
          (effectOrigin.includes(observer.id) ||
            effectName.includes(observer.name.toLowerCase()))
        ) {
    ...
    if (actor.getRollOptions) {
      ...
      for (const option of rollOptions) {
        ...
        if (
          (option.includes("self:hidden") || option.includes("hidden-to")) &&
          (option.includes(observer.id) ||
            option.includes(observer.name.toLowerCase()))
        ) {
          return "hidden";
        }
