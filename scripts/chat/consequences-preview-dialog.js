...
export class ConsequencesPreviewDialog extends foundry.applications.api.ApplicationV2 {
    ...
    async _prepareContext(options) {
        ...
        processedOutcomes = processedOutcomes.map(outcome => {
            ...
            return {
                ...
                tokenImage: outcome.target.document?.texture?.src || outcome.target.img || "icons/svg/mystery-man.svg"
            };
        });
        ...
        context.attackingToken = {
            ...this.attackingToken,
            image: this.attackingToken.document?.texture?.src || this.attackingToken.img || "icons/svg/mystery-man.svg"
        };
    ...
    static async _onApplyAll(event, target) {
        ...
        for (const outcome of changedOutcomes) {
            ...
            await applyVisibilityChanges(outcome.target, [{
        ...
        ui.notifications.info(`${MODULE_TITLE}: Applied all visibility changes. Dialog remains open for further adjustments.`);
    }
    ...
    async applyVisibilityChange(targetToken, newVisibility) {
        try {
            ...
            await applyVisibilityChanges(targetToken, [{
    ...
    updateBulkActionButtons() {
        const applyAllButton = this.element.querySelector('button.consequences-preview-dialog-bulk-action-btn.apply-all');
        const revertAllButton = this.element.querySelector('button.consequences-preview-dialog-bulk-action-btn.revert-all');
    ...
    markInitialSelections() {
        ...
        const allIcons = this.element.querySelectorAll('.state-selection .state-icon');
        ...
        this.outcomes.forEach(outcome => {
            ...
            if (row) {
                ...
                const selectedIcon = row.querySelector(`.state-selection .state-icon[data-state="${effectiveState}"]`);
                if (selectedIcon) {
                    // Add selected class
        ...
        setTimeout(() => {
            const initialIcons = this.element.querySelectorAll('.state-selection .state-icon.initial-outcome');
        ...
        icons.forEach(icon => {
            ...
            icon._clickHandler = (event) => {
                ...
                if (tokenId && state) {
                    ...
                    if (!outcome) {
                        console.error(`${MODULE_TITLE}: Could not find outcome for token ${tokenId}`);
                        return;
                    }
                    ...
                    outcome.hasActionableChange = this.calculateHasActionableChange(outcome);
                    ...
                    if (actionButtons) {
                        if (outcome.hasActionableChange) {
                            ...
                            actionButtons.style.display = 'flex';
                            actionButtons.innerHTML = '<span class="no-action">No change</span>';
                        }
                    }
                }
            };
