...
export function discoverDiversionObservers(divertingToken) {
  ...
  for (const token of canvas.tokens.placeables) {
    ...
    if (enforceRAW && shouldFilterAlly(divertingToken, token, "enemies"))
      continue;
...
export function analyzeDiversionOutcome(diversionData, observer) {
  ...
  if (!roll || typeof roll.total !== "number") {
    console.warn(
      `${MODULE_TITLE}: Invalid roll data in analyzeDiversionOutcome:`,
      roll,
    );
    return {
...
export async function previewDiversionResults(diversionData) {
  ...
  if (!diversionData || !diversionData.actor || !diversionData.roll) {
    console.error(
      `${MODULE_TITLE}: Invalid diversionData provided to previewDiversionResults:`,
      diversionData,
    );
    ui.notifications.error(
      `${MODULE_TITLE}: Invalid diversion data - cannot preview results`,
    );
    return;
  }
  ...
  if (observers.length === 0 && enforceRAW) {
    ...
    ui.notifications.info(
      `${MODULE_TITLE}: No creatures can see you, so Create a Diversion has no effect.`,
    );
    return;
  }
  ...
  const outcomes = observers.map((observer) =>
    analyzeDiversionOutcome(diversionData, observer),
  );
  ...
  if (changes.length === 0 && enforceRAW) {
    ...
    ui.notifications.info(
      `${MODULE_TITLE}: Create a Diversion roll did not result in any visibility changes.`,
    );
    return;
  }
