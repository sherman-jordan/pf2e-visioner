...
export class CreateADiversionPreviewDialog extends foundry.applications.api
  .ApplicationV2 {
  ...
  async _prepareContext(options) {
    ...
    context.divertingToken = {
      ...this.divertingToken,
      image:
        this.divertingToken.document?.texture?.src ||
        this.divertingToken.img ||
        "icons/svg/mystery-man.svg",
    };
  ...
  updateBulkActionButtons() {
    const applyAllButton = this.element.querySelector(
      "button.create-a-diversion-preview-dialog-bulk-action-btn.apply-all",
    );
    const revertAllButton = this.element.querySelector(
      "button.create-a-diversion-preview-dialog-bulk-action-btn.revert-all",
    );
