...
export class PointOutPreviewDialog extends foundry.applications.api
  .ApplicationV2 {
  ...
  async _prepareContext(options) {
    ...
    if (
      ...
      ui.notifications.info(
        `${MODULE_TITLE}: No encounter allies found, showing all`,
      );
    }
    ...
    const processedOutcomes = filteredOutcomes.map((outcome) => {
      const availableStates = {
        hidden: {
          ...
          selected:
            (outcome.overrideState || outcome.newVisibility) === "hidden",
        },
      };
      ...
      return {
        ...outcome,
        oldVisibilityState:
          visibilityStates[outcome.oldVisibility || outcome.currentVisibility],
        newVisibilityState: visibilityStates[outcome.newVisibility],
        tokenImage:
          outcome.target.texture?.src || outcome.target.document?.texture?.src,
    ...
    context.actorImage =
      this.actorToken.texture?.src || this.actorToken.document?.texture?.src;
  ...
  static async _onToggleEncounterFilter(event, button) {
    ...
    let allies = discoverPointOutAllies(
      app.actorToken,
      pointOutTarget,
      app.encounterOnly,
    );
    ...
    if (allies.length === 0 && app.encounterOnly) {
      ui.notifications.info(
        `${MODULE_TITLE}: No encounter allies found who can't see ${pointOutTarget.name} - showing all allies`,
      );
      app.encounterOnly = false;
      allies = discoverPointOutAllies(
        app.actorToken,
        pointOutTarget,
        app.encounterOnly,
      );
    ...
    if (allies.length === 0) {
      ui.notifications.info(
        `${MODULE_TITLE}: No allies found who can't see ${pointOutTarget.name}`,
      );
      return;
    }

    const outcomes = allies.map((allyData) =>
      analyzePointOutOutcome(app.actionData, allyData),
    );
  ...
  static async _onApplyChange(event, button) {
    ...
    try {
      ...
      const applyButton = app.element.querySelector(
        `button[data-action="applyChange"][data-token-id="${tokenId}"]`,
      );
      const revertButton = app.element.querySelector(
        `button[data-action="revertChange"][data-token-id="${tokenId}"]`,
      );
  ...
  updateChangesCount() {
    ...
    this.outcomes.forEach((outcome) => {
      const tokenId = outcome.target.id;
      const applyButton = this.element.querySelector(
        `button[data-action="applyChange"][data-token-id="${tokenId}"]`,
      );
      const revertButton = this.element.querySelector(
        `button[data-action="revertChange"][data-token-id="${tokenId}"]`,
      );
    ...
    // Update the count display
    const changesCountElement = this.element.querySelector(
    ...
    try {
      ...
      const promises = changes.map(async (change) => {
        try {
          if (!change.target) {
            console.error(
              `${MODULE_TITLE}: No ally token found in change:`,
              change,
            );
            return;
          }
          ...
          if (!pointedOutToken) {
            console.error(
              `${MODULE_TITLE}: Could not find pointed out token for Point Out`,
            );
            return;
          }
          ...
          console.error(
            `${MODULE_TITLE}: Error applying individual visibility change:`,
            error,
          );
        }
      });
      ...
      console.error(
        `${MODULE_TITLE}: Error applying visibility changes:`,
        error,
      );
      return false;
    }
  ...
  updateAllRowButtonsToApplied() {
    const applyButtons = this.element.querySelectorAll(
      ".row-action-btn.apply-change",
    );
    const revertButtons = this.element.querySelectorAll(
  ...
  updateRowButtonsToApplied(tokenId) {
    const applyButton = this.element.querySelector(
      `.row-action-btn.apply-change[data-token-id="${tokenId}"]`,
    );
    const revertButton = this.element.querySelector(
      `.row-action-btn.revert-change[data-token-id="${tokenId}"]`,
    );
      ...
      icon.addEventListener("click", (event) => {
        ...
        if (outcome) {
          ...
          outcome.hasActionableChange =
            outcome.overrideState !== currentVisibility;
