 ...
 */
export function discoverSeekTargets(
  seekerToken,
  encounterOnly = false,
  ...
  for (const token of canvas.tokens.placeables) {
    ...
    if (
      enforceRAW &&
      templateRadiusFeet != null &&
      Number.isFinite(templateRadiusFeet)
    ) {
      // If a template center was provided, calculate distance from template center to token
      if (
        templateCenter &&
        Number.isFinite(templateCenter.x) &&
        Number.isFinite(templateCenter.y)
      ) {
        try {
      ...
      const tokenRadiusFeet =
        ((token.document?.width ?? 1) * feetPerSquare) / 2;
      if (distance > templateRadiusFeet + tokenRadiusFeet) continue;
    }

    // Apply range limitation based on context and settings
    const usingTemplateArea =
      templateRadiusFeet != null && Number.isFinite(templateRadiusFeet);
    const isRangeLimited =
      !usingTemplateArea &&
      ((isInCombat && limitRangeInCombat) ||
        (!isInCombat && limitRangeOutOfCombat));
...
export function analyzeSeekOutcome(seekData, target) {
  ...
  if (outcome === "critical-success") {
    ...
    if (
      target.currentVisibility === "undetected" ||
      target.currentVisibility === "hidden"
    ) {
...
export async function previewSeekResults(actionData) {
  ...
  const hasExplicitTemplate = !!(
    actionData.seekTemplateCenter && actionData.seekTemplateRadiusFeet
  );
  const effectiveUseTemplate = useTemplate || hasExplicitTemplate;
  const rangeRestricted =
    !effectiveUseTemplate &&
    ((isInCombat && limitRangeInCombat) ||
      (!isInCombat && limitRangeOutOfCombat));
  ...
  const previewDialog = new SeekPreviewDialog(
    actionData.actor,
    outcomes,
    changes,
    actionData,
  ...
  if (templateRadiusFeet != null)
    previewDialog.templateRadiusFeet = templateRadiusFeet;
