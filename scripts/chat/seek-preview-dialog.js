...
export class SeekPreviewDialog extends foundry.applications.api.ApplicationV2 {
  ...
  _addHoverListeners() {
    // Selection-based row highlight parity with token manager
    ...
    const processedOutcomes = filteredOutcomes.map((outcome) => {
      ...
      const availableStates = [
        ...
        },
        {
          ...
          selected:
            (outcome.overrideState || outcome.newVisibility) === "observed",
  ...
  static async _onApplyAll(event, button) {
    ...
    try {
      ...
      ui.notifications.info(
        `${MODULE_TITLE}: Applied ${changes.length} visibility changes. Dialog remains open for additional actions.`,
      );
  ...
  static async _onRevertChange(event, button) {
    ...
    try {
      ...
      ui.notifications.info(
        `${MODULE_TITLE}: Reverted ${outcome.target.name} to original visibility`,
      );
  ...
  static async _onToggleEncounterFilter(event, button) {
    ...
    if (targets.length === 0) {
      ui.notifications.info(
        `${MODULE_TITLE}: No ${app.encounterOnly ? "encounter " : ""}targets found for seek action`,
      );
    ...
    const outcomes = targets.map((target) =>
      analyzeSeekOutcome(app.actionData, target),
    );
