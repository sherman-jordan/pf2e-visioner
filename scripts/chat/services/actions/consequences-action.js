...
export class ConsequencesActionHandler extends ActionHandlerBase {
  ...
  entriesToRevertChanges(entries, actionData) {
    return entries
      .map((e) => ({ observer: this.getTokenById(e.observerId), target: actionData?.actor || null, newVisibility: e.oldVisibility }))
  ...
  async fallbackRevertChanges(actionData) {
    ...
    return filtered.map((o) => ({ observer: o.target, target: actionData.actor, newVisibility: o.currentVisibility }));
  }
