  ...
  async _prepareContext(options) {
    ...
    if (
      ...
      ui.notifications.info(
        `${MODULE_TITLE}: No encounter observers found, showing all`,
      );
    }
    ...
    const processedOutcomes = filteredOutcomes.map((outcome) => {
      ...
      const availableStates = [
        ...
        },
        {
          ...
          selected:
            (outcome.overrideState || outcome.newVisibility) === "undetected",
      ...
      const hasActionableChange =
        outcome.newVisibility !== outcome.oldVisibility;
      ...
      return {
        ...
        oldVisibilityState:
          visibilityStates[outcome.oldVisibility || currentVisibility],
        newVisibilityState: visibilityStates[outcome.newVisibility],
        marginText:
          outcome.margin >= 0 ? `+${outcome.margin}` : `${outcome.margin}`,
        tokenImage:
          outcome.token.texture?.src || outcome.token.document?.texture?.src,
    ...
    context.sneaker = {
      ...
      image:
        this.sneakingToken.texture?.src ||
        this.sneakingToken.document?.texture?.src,
      actionType: "sneak",
  ...
  markInitialSelections() {
    this.outcomes.forEach((outcome) => {
      ...
      const row = this.element.querySelector(
        `tr[data-token-id="${outcome.token.id}"]`,
      );
      if (row) {
  ...
  static async _onApplyAll(event, button) {
    ...
    for (const outcome of changedOutcomes) {
      ...
      try {
        ...
        console.warn(
          "Error applying visibility changes for bulk apply:",
          error,
        );
  ...
  static async _onRevertAll(event, button) {
    ...
    for (const outcome of changedOutcomes) {
      ...
      try {
        ...
        console.warn(
          "Error reverting visibility changes for bulk revert:",
          error,
        );
        // Continue with other outcomes even if one fails
      }
