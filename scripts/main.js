...
function updateColorblindMode() {
  try {
    ...
    if (colorblindMode !== "none") {
      ...
      console.log("PF2E Visioner: No colorblind mode active, removed all colorblind classes from body");
    }
    ...
    containers.forEach((container, index) => {
      ...
      if (colorblindMode !== "none") {
        ...
        console.log(`PF2E Visioner: Applied colorblind mode "${colorblindMode}" to container ${index}`);
      }
    });
    ...
    if (colorblindMode !== "none") {
      setTimeout(() => {
        const bodyHasClass = document.body.classList.contains(`pf2e-visioner-colorblind-${colorblindMode}`);
        const containersHaveClass = Array.from(document.querySelectorAll('.pf2e-visioner')).some(container => 
          container.classList.contains(`pf2e-visioner-colorblind-${colorblindMode}`)
        );
        console.log(`PF2E Visioner: Verification - Body has colorblind class: ${bodyHasClass}, Containers have colorblind class: ${containersHaveClass}`);
        ...
        console.log(`PF2E Visioner: CSS Custom Property --visibility-observed-color: "${visibilityColor}"`);
        ...
        if (firstContainer) {
          ...
          const containerVisibilityColor = containerStyle.getPropertyValue('--visibility-observed-color');
          console.log(`PF2E Visioner: Container CSS Custom Property --visibility-observed-color: "${containerVisibilityColor}"`);
        }
      }, 100);
    }
  } catch (error) {
...
Hooks.on("renderApplication", (app, html) => {
  ...
  if (html && html[0] && html[0].classList && html[0].classList.contains('pf2e-visioner')) {
    ...
    const colorblindMode = game.settings.get("pf2e-visioner", "colorblindMode");
    console.log(`PF2E Visioner: Applying colorblind mode "${colorblindMode}" to application`, app.constructor.name);
    ...
    if (colorblindMode !== "none") {
      ...
      console.log(`PF2E Visioner: Added class "pf2e-visioner-colorblind-${colorblindMode}" to`, html[0]);
    }
  }
});
