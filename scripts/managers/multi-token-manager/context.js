...
export async function buildMultiTokenContext(app, options) {
  ...
  const context = BaseApp
    ? await BaseApp.prototype._prepareContext.call(app, options)
    : {};
  ...
  const targets = allSceneTargets.filter(token => 
    token.id !== currentToken.id
  );
  
  // Debug logging (remove in production)
  // console.log("Multi Token Manager Context Debug:", {
  //   currentToken: currentToken.name,
  //   totalSceneTargets: allSceneTargets.length,
  //   filteredTargets: targets.length,
  //   selectedTokens: app.selectedTokens.length,
  ...
  const allTargets = targets.map((token) => {
    ...
    const currentVisibilityState = savedChanges?.visibility[token.document.id] || baseVisibilityState;
  ...
  // More debug logging (remove in production)
  // console.log("Target categorization:", {
  //   allTargets: allTargets.length,
  //   pcTargets: context.pcTargets.length,
  //   npcTargets: context.npcTargets.length,
  //   lootTargets: context.lootTargets.length,
  //   firstTarget: allTargets[0] ? {
  //     name: allTargets[0].name,
  //     isPC: allTargets[0].isPC,
  //     isLoot: allTargets[0].isLoot
  //   } : "none"
  // });
