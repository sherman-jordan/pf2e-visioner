...
async function removeObserverFromAggregate(
  ...
  const aggregate = effects.find(
    (e) =>
      ...
      e.flags?.[MODULE_ID]?.effectTarget ===
    ...
    if (empties.length) {
      ...
      if (ids.length) {
        try {
          ...
          console.error(
            `${LOG_PREFIX}: Error pruning empty aggregates:`,
            error,
          );
        }
      }
 ...
 */
export async function cleanupEphemeralEffectsForTarget(
  observerToken,
  hiddenToken,
) {
  ...
  try {
    ...
    const ephemeralEffects = observerToken.actor.itemTypes.effect.filter(
      (e) =>
        ...
        e.flags?.[MODULE_ID]?.hiddenActorSignature ===
          hiddenToken.actor.signature,
    );

    if (ephemeralEffects.length > 0) {
      const effectIds = ephemeralEffects.map((e) => e?.id).filter((id) => !!id);
      const existingIds = effectIds.filter(
        (id) => !!observerToken?.actor?.items?.get?.(id),
      );

      if (existingIds.length > 0) {
        try {
          // Delete all effects in a single bulk operation
          await observerToken.actor.deleteEmbeddedDocuments(
            "Item",
            existingIds,
          );
        } catch (error) {
          console.error(
            `${LOG_PREFIX}: Error bulk deleting observer effects:`,
            error,
          );
    ...
    try {
        ...
        if (ids.length) {
          try {
            ...
            if (game.settings?.get?.("pf2e-visioner", "debug")) {
              console.warn(
                "[Visioner-debug] Bulk delete legacy off-guard on target",
                { target: hiddenToken.name, ids },
              );
            }
            await hiddenToken.actor.deleteEmbeddedDocuments("Item", ids);
          } catch (error) {
            console.error(
              `${LOG_PREFIX}: Error deleting legacy off-guard effects:`,
              error,
            );
          }
        }
      ...
      console.error(
        `${LOG_PREFIX}: Error processing legacy target effects:`,
        error,
      );
    }
    ...
    try {
      ...
      console.error(
        `${LOG_PREFIX}: Error removing observer from aggregates:`,
        error,
      );
    }
  } catch (error) {
    console.error(
      `${LOG_PREFIX}: Error cleaning up ephemeral effects for target:`,
      error,
    );
  }
}
...
export async function cleanupDeletedTokenEffects(tokenDoc) {
  ...
  console.log(
    `[${MODULE_ID}] Cleaning up effects for deleted token:`,
    tokenDoc.name,
    tokenDoc.id,
  );
  ...
  try {
    ...
    console.log(
      `[${MODULE_ID}] Checking ${allTokens.length} tokens for effects referencing deleted token`,
    );
    ...
    for (let i = 0; i < allTokens.length; i += batchSize) {
      ...
      for (const token of batch) {
        ...
        if (observerEffects.length > 0) {
          console.log(
            `[${MODULE_ID}] Found ${observerEffects.length} effects where deleted token is the observer`,
          );
        ...
        console.log(
          `[${MODULE_ID}] Found ${relevantEffects.length} aggregate effects on token ${token.name}`,
        );
        ...
        for (const effect of relevantEffects) {
          ...
          console.log(
            `[${MODULE_ID}] Checking ${rules.length} rules in effect ${effect.name}`,
          );
          ...
          const newRules = rules.filter((r) => {
            ...
            if (
              ...
              console.log(
                `[${MODULE_ID}] Found reference to deleted token in rule:`,
                r,
              );
              return false;
            }
          ...
          if (newRules.length !== rules.length) {
            console.log(
              `[${MODULE_ID}] Rules changed: ${rules.length} -> ${newRules.length}`,
            );
            ...
            if (newRules.length === 0) {
              ...
              console.log(
                `[${MODULE_ID}] Marking effect ${effect.name} for deletion`,
              );
            } else {
              ...
              console.log(
                `[${MODULE_ID}] Marking effect ${effect.name} for update with ${newRules.length} rules`,
              );
            }
          } else {
            console.log(
              `[${MODULE_ID}] No rules changed for effect ${effect.name}`,
            );
          }
        }
        ...
        if (legacyEffects.length > 0) {
          console.log(
            `[${MODULE_ID}] Found ${legacyEffects.length} legacy effects referencing deleted token`,
          );
          effectsToDelete.push(...legacyEffects.map((e) => e.id));
        }

        console.log(
          `[${MODULE_ID}] Final counts: ${effectsToDelete.length} to delete, ${effectsToUpdate.length} to update`,
        );
        ...
        try {
          ...
          if (effectsToDelete.length > 0) {
            console.log(
              `[${MODULE_ID}] Deleting ${effectsToDelete.length} effects for token ${token.name}`,
            );
            await token.actor.deleteEmbeddedDocuments("Item", effectsToDelete);
            console.log(
              `[${MODULE_ID}] Successfully deleted ${effectsToDelete.length} effects`,
            );
          }
          ...
          if (effectsToUpdate.length > 0) {
            console.log(
              `[${MODULE_ID}] Updating ${effectsToUpdate.length} effects for token ${token.name}`,
            );
            console.log(`[${MODULE_ID}] Update data:`, effectsToUpdate);
            await token.actor.updateEmbeddedDocuments("Item", effectsToUpdate);
            console.log(
              `[${MODULE_ID}] Successfully updated ${effectsToUpdate.length} effects`,
            );
          }
        } catch (error) {
          console.error(
            `${LOG_PREFIX}: Error updating effects for deleted token:`,
            error,
          );
        }
      }
    ...
    console.error(
      `${LOG_PREFIX}: Error cleaning up effects for deleted token:`,
      error,
    );
  }
}
...
export async function updateEphemeralEffectsForVisibility(
  ...
  if (!observerToken?.actor || !targetToken?.actor) {
    console.warn(`${LOG_PREFIX}: missing actor(s)`, {
      observer: observerToken?.id,
  ...
  try {
    const receiverType =
      (effectTarget === "observer"
        ? observerToken?.actor?.type
        : targetToken?.actor?.type) || null;
  ...
  await runWithEffectLock(effectReceiverToken.actor, async () => {
    ...
    try {
      ...
      if (effectsToUpdate.length > 0) {
        await effectReceiverToken.actor.updateEmbeddedDocuments(
          "Item",
          effectsToUpdate,
        );
      }
      ...
      if (effectsToCreate.length > 0) {
        await effectReceiverToken.actor.createEmbeddedDocuments(
          "Item",
          effectsToCreate,
        );
      }
 ...
 * @param {Object} options - Optional configuration
 */
export async function batchUpdateVisibilityEffects(
  observerToken,
  targetUpdates,
  options = {},
) {
      ...
      try {
        ...
        for (const { source, state } of updates) {
          ...
          if (operations.hidden.remove) {
            const filteredRules = hiddenRules.filter(
              (r) =>
                !(
                  r?.key === "EphemeralEffect" &&
                  Array.isArray(r.predicate) &&
          ...
          if (operations.undetected.remove) {
            const filteredRules = undetectedRules.filter(
              (r) =>
                !(
                  r?.key === "EphemeralEffect" &&
                  Array.isArray(r.predicate) &&
...
function createAggregateEffectData(visibilityState, signature, options = {}) {
  const visibilityLabel = game.i18n.localize(
    `PF2E.condition.${visibilityState}.name`,
  );
