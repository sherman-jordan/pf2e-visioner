...
import { setVisibilityBetween, getVisibility } from '../stores/visibility-map.js';
import { isValidToken } from '../utils.js';
import { segmentsIntersect } from '../helpers/geometry-utils.js';

const RegionBehaviorBase = (typeof foundry !== 'undefined' && foundry.data && foundry.data.regionBehaviors && foundry.data.regionBehaviors.RegionBehaviorType)
  ...
  static LOCALIZATION_PREFIXES = ["PF2E_VISIONER.REGION_BEHAVIOR"];
    ...
    async _handleRegionEvent(event) {
      ...
      try {
        ...
        else if (event.target && typeof event.target === 'object' && event.target?.id) token = event.target;
        else if (event && typeof event === 'object' && event?.id && event?.center) token = event;
        else if (event?.data?.token) token = canvas.tokens.get(event.data.token?.id ?? event.data.token);
  
        // Additional common shapes for combat/turn/round events
        if (!token && event?.tokenId) token = canvas.tokens.get(event.tokenId);
        if (!token && event?.token && typeof event.token === 'string') token = canvas.tokens.get(event.token);
        if (!token && event?.actor && event.actor?.token) token = event.actor.token;
        if (!token && event?.actor && event.actor?.tokenId) token = canvas.tokens.get(event.actor.tokenId);
      ...
      else isEntering = [CONST.REGION_EVENTS.TOKEN_ENTER, CONST.REGION_EVENTS.TOKEN_ANIMATE_IN, CONST.REGION_EVENTS.TOKEN_MOVE_IN].includes(name);
    ...
    _prepareWallCache() {
      return (canvas?.walls?.placeables || []).map(w => {
        const d = w.document;
        let wx1 = d?.x, wy1 = d?.y, wx2 = d?.x2, wy2 = d?.y2;
        if ((wx1 === undefined || wy1 === undefined || wx2 === undefined || wy2 === undefined) && Array.isArray(d?.c) && d.c.length >= 4) {
          wx1 = d.c[0]; wy1 = d.c[1]; wx2 = d.c[2]; wy2 = d.c[3];
        }
    ...
    _gatherUpdatesForToken(tokenId, isEntering, tokensInRegion) {
      ...
      if (isEntering) {
        ...
        for (let i = inRegion.length - 1; i >= 0; --i) if (inRegion[i].id === token.id) inRegion.splice(i, 1);
      }
    const allTokens = canvas.tokens.placeables.filter(t => isValidToken(t) && !ignoredActorTypes.has(getTokenActorType(t)));
  ...
  if (isEntering) {
        ...
        try {
          if (game && game.combat && game.combat.combatant) {
            const currentCombatant = game.combat.combatant?.token?.id ?? game.combat.combatant?.tokenId ?? null;
            if (currentCombatant) {
              ...
              if (tokensInRegion && tokensInRegion.find(t => t.id === currentCombatant)) skipResetDueToCombat = true;
            }
          }
        } catch (_) {}

        if (!skipResetDueToCombat) {
          const tokensToReset = [...inRegion.filter(t => t.id !== token.id), ...allTokens.filter(t => !inRegion.includes(t) && t.id !== token.id)];
    ...
    async _processPendingEvents() {
      ...
      try {
        ...
        for (const e of entries) {
          allUpdates = allUpdates.concat(this._gatherUpdatesForToken(e.id, e.isEntering, tokensInRegion));
        }
        ...
        for (const u of allUpdates) {
          ...
          if (!seen.has(key)) { seen.add(key); uniqueUpdates.push(u); }
        }
        ...
        const segBBox = (p1, p2) => ({ minX: Math.min(p1.x, p2.x), maxX: Math.max(p1.x, p2.x), minY: Math.min(p1.y, p2.y), maxY: Math.max(p1.y, p2.y) });
          ...
          try {
            ...
            for (const w of walls) {
              try {
                ...
                if (w.xmax < segMinX || w.xmin > segMaxX || w.ymax < segMinY || w.ymin > segMaxY) continue;
                if (segmentsIntersect(p1, p2, { x: w.wx1, y: w.wy1 }, { x: w.wx2, y: w.wy2 })) return false;
              } catch (_) { continue; }
            }
          } catch (_) { /* ignore */ }
          return true;
    ...
    async _applyVisibilityToToken(token) {
      const visibilityState = this.visibilityState;
      const applyToInsideTokens = this.applyToInsideTokens;
      const twoWayRegion = this.twoWayRegion;
  
    const tokensInRegion = this._getTokensInRegion();
    // Prefilter tokens by validity once
    const allTokens = canvas.tokens.placeables.filter(t => isValidToken(t));
      const tokensOutsideRegion = allTokens.filter(t => !tokensInRegion.includes(t));
    ...
    const updates = [];
      ...
      if (applyToInsideTokens) {
        for (const insideA of tokensInRegion) {
          for (const insideB of tokensInRegion) {
            if (insideA.id === insideB.id) continue;
            updates.push({ source: insideA.id, target: insideB.id, state: visibilityState });
          }
        }
      ...
      // Precompute wall segments + bounding boxes to reduce work inside token loops
      const walls = (canvas?.walls?.placeables || []).map(w => {
        const d = w.document;
        let wx1 = d?.x, wy1 = d?.y, wx2 = d?.x2, wy2 = d?.y2;
        if ((wx1 === undefined || wy1 === undefined || wx2 === undefined || wy2 === undefined) && Array.isArray(d?.c) && d.c.length >= 4) {
          wx1 = d.c[0]; wy1 = d.c[1]; wx2 = d.c[2]; wy2 = d.c[3];
      ...
      const isLineOfSightClear = (p1, p2) => {
        try {
          // Segment bbox to early-reject walls
      ...
      // Deduplicate updates: avoid repeated identical source/target/state triples
      const seen = new Set();
      const uniqueUpdates = [];
    ...
    // dedup done - we track final filtered length below
      // Cache centers to avoid repeated property lookups
      const centerCache = new Map();
      const getCenter = (id) => {
        if (centerCache.has(id)) return centerCache.get(id);
        const t = canvas.tokens.get(id);
    ...
    async _removeVisibilityFromToken(token) {
      const tokensInRegion = this._getTokensInRegion().filter(t => t.id !== token.id);
      const allTokens = canvas.tokens.placeables.filter(t => isValidToken(t));
      const tokensOutsideRegion = allTokens.filter(t => !tokensInRegion.includes(t) && t.id !== token.id);
  
      const updates = [];
      for (const otherToken of [...tokensInRegion, ...tokensOutsideRegion]) {
        updates.push({ source: token.id, target: otherToken.id, state: 'observed' });
      }
      for (const otherToken of [...tokensInRegion, ...tokensOutsideRegion]) {
        updates.push({ source: otherToken.id, target: token.id, state: 'observed' });
      ...
      await this._applyVisibilityUpdates(updates);
    }
    ...
    _getTokensInRegion() {
      ...
      const pointInRegion = (x, y) => {
        try {
          ...
          if (typeof region.containsPoint === 'function') {
            try { if (region.containsPoint({ x, y })) return true; } catch (__) {}
            try { if (region.containsPoint(x, y)) return true; } catch (__) {}
          }
          if (region.shape && typeof region.shape.containsPoint === 'function') {
            try { if (region.shape.containsPoint(new PIXI.Point(x, y))) return true; } catch (__) {}
          }
      ...
      return canvas.tokens.placeables.filter(token => {
        ...
        if (regionBBox) {
          ...
          if (x < (regionBBox.left ?? -Infinity) || x > (regionBBox.right ?? Infinity) || y < (regionBBox.top ?? -Infinity) || y > (regionBBox.bottom ?? Infinity)) return false;
        }
      ...
      try {
        ...
        for (let i = 0; i < updates.length; i += batchSize) {
          ...
          for (const update of batch) {
            ...
            try {
              ...
              if (!sourceToken || !targetToken) {
                console.warn('PF2e Visioner | Could not resolve source/target tokens for update', update);
                continue;
              }
              ...
              resolved += 1;
              promises.push(setVisibilityBetween(sourceToken, targetToken, state).catch(err => ({ err, update })));
            } catch (err) {
          ...
          if (promises.length) {
            ...
            for (const r of results) if (r.status === 'rejected' || (r.status === 'fulfilled' && r.value && r.value.err)) console.error('PF2e Visioner | Error applying visibility update:', r);
          }
          if (resolved === 0) console.warn('PF2e Visioner | No updates in this batch were resolved to canvas tokens', batch);
          if (i + batchSize < updates.length) await new Promise(resolve => setTimeout(resolve, 10));
        }
  
        try {
          if (typeof canvas !== 'undefined' && canvas?.perception?.update) canvas.perception.update({ refreshVision: true });
        } catch (err) {
