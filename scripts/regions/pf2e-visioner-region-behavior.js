...
export class PF2eVisionerRegionBehavior extends RegionBehaviorBase {
  ...
  async _handleRegionEvent(event) {
    ...
    const isEntering = [CONST.REGION_EVENTS.TOKEN_ENTER, CONST.REGION_EVENTS.TOKEN_ANIMATE_IN, CONST.REGION_EVENTS.TOKEN_MOVE_IN].includes(name);
  // exiting is handled by scheduling with isEntering=false; no immediate use here

    try {
      if (token) this._scheduleTokenEvent(token, isEntering);
    } catch (error) {
  ...
  _scheduleTokenEvent(token, isEntering) {
    this._ensurePending();
    try {
      this._pendingTokenEvents.set(token.id, { id: token.id, isEntering });
