...
let VisibilityRuleElement = null;
...
function setupRuleElements() {
  ...
  if (game.pf2e?.RuleElementPF2e) {
    try {
      // Define the VisibilityRuleElement class
      VisibilityRuleElement = class extends game.pf2e.RuleElementPF2e {
        /**
        ...
        static get documentation() {
          return "https://github.com/roileaf/pf2e-visioner/blob/main/RULE_ELEMENTS.md#visibility-rule-element";
        }
      ...
      console.log('PF2E Visioner | Rule element class defined successfully');
    } catch (error) {
      console.error('PF2E Visioner | Error defining rule element class:', error);
    }
  } else {
    console.warn('PF2E Visioner | PF2e system not ready during setup, will try again during ready hook');
  }
}
...
function registerRuleElements() {
  ...
  try {
    ...
    if (!VisibilityRuleElement && game.pf2e?.RuleElementPF2e) {
      console.log('PF2E Visioner | Defining rule element class during ready hook');
      setupRuleElements();
    }
    
    if (!VisibilityRuleElement) {
      console.error('PF2E Visioner | Failed to create VisibilityRuleElement class');
      return;
    }
