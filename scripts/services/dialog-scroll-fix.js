...
function applyScrollFix(element) {
  ...
  const contentWrapper = windowContent.querySelector(
  ...
  const tableContainer = contentWrapper.querySelector(
  ...
  // Get the footer
  const footer = contentWrapper.querySelector(
...
function getDialogType(dialogWindow) {
  ...
  if (dialogWindow.classList.contains("point-out-preview-dialog"))
    return "point-out";
  if (dialogWindow.classList.contains("sneak-preview-dialog")) return "sneak";
  if (dialogWindow.classList.contains("create-a-diversion-preview-dialog"))
