                        ...
                        const animate = () => {
                            try {
                                ...
                                sparkles.forEach((sparkle, i) => {
                                    ...
                                    const floatX = (sparkle._floatRange * (0.6 * Math.sin(curveTimeX * 2.1) + 0.3 * Math.sin(curveTimeX * 3.7) + 0.1 * Math.sin(curveTimeX * 6.2))) / 3;
                                    const floatY = (sparkle._floatRange * (0.6 * Math.cos(curveTimeY * 1.8) + 0.3 * Math.cos(curveTimeY * 4.1) + 0.1 * Math.cos(curveTimeY * 5.9))) / 3;
        ...
        if (isGM && (updates.length > 0 || tokenWallFlagUpdates.length > 0)) {
            try {
                ...
                if (tokenWallFlagUpdates.length > 0)
                    await canvas.scene?.updateEmbeddedDocuments?.('Token', tokenWallFlagUpdates, { diff: false });
