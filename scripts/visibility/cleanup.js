...
async function removeObserverFromAggregate(effectReceiverToken, observerToken, visibilityState, options = {}) {
  ...
  const rules = Array.isArray(aggregate.system.rules)
    ? aggregate.system.rules.filter(
        (r) => !(r?.key === "EphemeralEffect" && Array.isArray(r.predicate) && r.predicate.includes(`target:signature:${signature}`)),
      )
    : [];
  if (rules.length === 0) {
    try {
...
async function pruneEmptyAggregates(effectReceiverToken) {
  ...
  try {
    ...
    if (empties.length) {
      ...
      if (ids.length) {
        try { await effectReceiverToken.actor.deleteEmbeddedDocuments("Item", ids); } catch (error) {
...
export async function cleanupEphemeralEffectsForTarget(observerToken, hiddenToken) {
  ...
  try {
    const ephemeralEffects = observerToken.actor.itemTypes.effect.filter(
      (e) => e.flags?.[MODULE_ID]?.isEphemeralOffGuard && e.flags?.[MODULE_ID]?.hiddenActorSignature === hiddenToken.actor.signature,
    );
    if (ephemeralEffects.length > 0) {
      ...
      if (existingIds.length > 0) {
        try { await observerToken.actor.deleteEmbeddedDocuments("Item", existingIds); } catch (error) {
          console.error(`[${MODULE_ID}] Error bulk deleting observer effects:`, error);
          for (const id of existingIds) { if (!!id && !!observerToken?.actor?.items?.get?.(id)) { try { await observerToken.actor.deleteEmbeddedDocuments("Item", [id]); } catch (_) {} } }
        }
      }
    }
    try {
      const legacyOnTarget = hiddenToken.actor.itemTypes.effect.filter(
        (e) => e.flags?.[MODULE_ID]?.isEphemeralOffGuard && e.flags?.[MODULE_ID]?.hiddenActorSignature === observerToken.actor.signature,
      );
      if (legacyOnTarget.length) {
        const ids = legacyOnTarget.map((e) => e.id).filter((id) => !!hiddenToken.actor.items.get(id));
        if (ids.length) {
          try { await hiddenToken.actor.deleteEmbeddedDocuments("Item", ids); } catch (error) { console.error(`[${MODULE_ID}] Error deleting legacy off-guard effects:`, error); }
        }
      }
    } catch (error) {
  ...
  try {
    ...
    for (let i = 0; i < allTokens.length; i += batchSize) {
      ...
      for (const token of batch) {
        ...
        const observerEffects = effects.filter((e) => e.flags?.[MODULE_ID]?.aggregateOffGuard === true && e.flags?.[MODULE_ID]?.observerToken === tokenId);
        if (observerEffects.length > 0) { effectsToDelete.push(...observerEffects.map((e) => e.id)); continue; }
        const relevantEffects = effects.filter((e) => e.flags?.[MODULE_ID]?.aggregateOffGuard === true);
        for (const effect of relevantEffects) {
          const rules = Array.isArray(effect.system?.rules) ? [...effect.system.rules] : [];
          const newRules = rules.filter((r) => { const s = JSON.stringify(r); if (s.includes(signature) || s.includes(tokenId)) { return false; } return true; });
          if (newRules.length !== rules.length) {
            if (newRules.length === 0) { effectsToDelete.push(effect.id); } else { effectsToUpdate.push({ _id: effect.id, "system.rules": newRules }); }
          }
        }
        const legacyEffects = effects.filter((e) => e.flags?.[MODULE_ID]?.offGuard === true && (e.flags?.[MODULE_ID]?.observerToken === tokenId || e.flags?.[MODULE_ID]?.targetToken === tokenId));
        if (legacyEffects.length > 0) { effectsToDelete.push(...legacyEffects.map((e) => e.id)); }
        try {
          if (effectsToDelete.length > 0) { await token.actor.deleteEmbeddedDocuments("Item", effectsToDelete); }
          if (effectsToUpdate.length > 0) { await token.actor.updateEmbeddedDocuments("Item", effectsToUpdate); }
        } catch (error) { console.error(`${MODULE_ID}: Error updating effects for deleted token:`, error); }
      }
    }
  } catch (error) {
