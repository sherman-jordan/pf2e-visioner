...
describe('Auto-Cover Complete Workflow Integration', () => {
  ...
  beforeEach(() => {
    ...
    global.game = {
      ...
      keybindings: {
        get: jest.fn().mockReturnValue([{ key: 'KeyX', modifiers: [] }]),
      },
      scenes: {
        current: {
          id: 'test-scene',
  ...
  describe('Complete Auto-Cover Detection Workflow', () => {
    test('processes attack messages without errors', async () => {
      ...
      await expect(
        onPreCreateChatMessage(chatMessage, messageData, options, userId),
    ...
    test('handles corrupted chat message data', async () => {
      ...
      await expect(
        onPreCreateChatMessage(corruptedMessage, {}, {}, 'user'),
      ).resolves.toBeUndefined();
    });
  });
