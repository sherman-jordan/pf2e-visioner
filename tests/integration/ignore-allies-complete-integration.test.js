...
describe('Ignore Allies Complete Integration', () => {
  ...
  describe('2. Token Manager Class Integration', () => {
    ...
    test('CRITICAL: toggleIgnoreAllies should be registered in ApplicationV2 actions', async () => {
      ...
      expect(defaultOptions.actions.toggleIgnoreAllies).toBe(VisionerTokenManager.toggleIgnoreAllies);
      
      console.log('✅ CRITICAL TEST PASSED: toggleIgnoreAllies is registered in ApplicationV2 actions');
    });
  ...
  describe('3. Template Integration', () => {
    ...
    test('all preview templates should have ignore allies checkbox', async () => {
      ...
      console.log(`✅ Found ignore allies checkbox in ${templatesWithIgnoreAllies} preview templates`);
    });
  });
  ...
  describe('5. Regression Prevention', () => {
    ...
    test('should verify all toggle actions are consistently registered', async () => {
      ...
      const actions = VisionerTokenManager.DEFAULT_OPTIONS.actions;
      const toggleActions = ['toggleMode', 'toggleEncounterFilter', 'toggleIgnoreAllies', 'toggleTab'];
  ...
  describe('6. Future-Proofing', () => {
    test('should detect if new toggle actions are added but not registered', async () => {
      ...
      const staticMethods = Object.getOwnPropertyNames(VisionerTokenManager)
        .filter(name => name.startsWith('toggle') && typeof VisionerTokenManager[name] === 'function');
      
      const registeredActions = Object.keys(VisionerTokenManager.DEFAULT_OPTIONS.actions)
      ...
      console.log(`✅ Future-proofing: All ${staticMethods.length} toggle methods are properly registered`);
    });
  });
});
