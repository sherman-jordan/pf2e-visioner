...
describe('Ignore Allies REAL Import Chain Test', () => {
  ...
  test('CRITICAL: utils.js must import shouldFilterAlly directly (not via global)', async () => {
    ...
    expect(utilsContent).toContain('import { shouldFilterAlly } from "./chat/services/infra/shared-utils.js"');
  ...
  test('FAILURE TEST: This test should FAIL if the import line is removed', async () => {
    ...
    const hasDirectImport = utilsContent.includes('import { shouldFilterAlly } from "./chat/services/infra/shared-utils.js"');
    
    if (!hasDirectImport) {
      throw new Error('CRITICAL: Direct import of shouldFilterAlly is missing from utils.js - ignore allies will not work!');
    }
