...
describe('Chat Message Updates and Error Handling Tests', () => {
  ...
  describe('Chat Message Update Bug Fix', () => {
    test('should detect missing UI panels correctly', () => {
      ...
      const hasVisionerUI1 = htmlWithPanels.find && htmlWithPanels.find('.pf2e-visioner-automation-panel').length > 0;
      const hasVisionerUI2 = htmlWithoutPanels.find && htmlWithoutPanels.find('.pf2e-visioner-automation-panel').length > 0;
    ...
    test('should handle missing find method gracefully', () => {
      ...
      const hasVisionerUI = htmlWithoutFind.find && htmlWithoutFind.find('.pf2e-visioner-automation-panel').length > 0;
  ...
  describe('Player Error Handling Tests', () => {
    ...
    test('should not show console errors to players during token deletion', async () => {
      ...
      jest.doMock('../../scripts/services/scene-cleanup.js', () => ({
    ...
    test('should handle party token consolidation errors gracefully for players', async () => {
      ...
      const mockRestoreTokenStateFromParty = jest.fn().mockImplementation(async (tokenDoc, scene) => {
        try {
      ...
      jest.doMock('../../scripts/services/party-token-state.js', () => ({
      ...
      await expect(mockRestoreTokenStateFromParty(validToken.document, canvas.scene)).resolves.toBe(true);
      ...
      expect(console.warn).toHaveBeenCalledWith('Party token restoration failed:', 'Invalid token document');
    });

    test('should handle visibility effect errors gracefully for players', async () => {
      // Mock visibility effects service
      const mockBatchUpdateVisibilityEffects = jest.fn().mockImplementation(async (observer, targets) => {
        try {
          if (!observer?.actor) {
      ...
      jest.doMock('../../scripts/visibility/ephemeral.js', () => ({
      ...
      expect(console.warn).toHaveBeenCalledWith('Visibility effect update failed:', 'Target target has no actor');
    });

    test('should handle cover effect errors gracefully for players', async () => {
      // Mock cover effects service
      const mockBatchUpdateCoverEffects = jest.fn().mockImplementation(async (observer, targets) => {
        try {
      ...
      jest.doMock('../../scripts/cover/ephemeral.js', () => ({
      ...
      expect(console.warn).toHaveBeenCalledWith('Cover effect update failed:', 'Invalid target token: invalid');
    });
    ...
    test('should handle race conditions during mass token operations gracefully', async () => {
      ...
      const mockCleanupDeletedToken = jest.fn().mockImplementation((token) => {
        try {
          ...
          if (Math.random() > 0.8) { // 20% chance of race condition
