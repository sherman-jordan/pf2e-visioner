...
describe('Party Token Integration Tests', () => {
  ...
  describe('Party Token State Saving', () => {
    ...
    test('should save module effects correctly', () => {
      ...
      const moduleEffects = token.actor.items.filter(item => 
        item.flags?.['pf2e-visioner']?.createdBy === 'pf2e-visioner'
      );
  ...
  describe('Party Token State Restoration', () => {
    ...
    test('should restore effects FROM restored player tokens TO existing NPCs on canvas', () => {
      ...
      const observerVisTargets = global.canvas.tokens.placeables
        .filter(t => t.id !== restoredPlayer.id && restoredVisibilityMap[t.id] && restoredVisibilityMap[t.id] !== "observed")
        .map(t => ({ target: t, state: restoredVisibilityMap[t.id] }));
      
      const observerCovTargets = global.canvas.tokens.placeables
        .filter(t => t.id !== restoredPlayer.id && restoredCoverMap[t.id] && restoredCoverMap[t.id] !== "none")
      ...
      expect(mockBatchUpdateVisibilityEffects).toHaveBeenCalledWith(restoredPlayer, observerVisTargets);
    ...
    test('should restore effects FROM existing NPCs TO restored player tokens', () => {
      ...
      for (const observer of allTokens) {
        ...
        if (visMap[restoredPlayer.id] && visMap[restoredPlayer.id] !== "observed") {
          ...
          mockBatchUpdateVisibilityEffects(observer, [{ target: restoredPlayer, state: visMap[restoredPlayer.id] }]);
        }
        ...
        if (covMap[restoredPlayer.id] && covMap[restoredPlayer.id] !== "none") {
          ...
          mockBatchUpdateCoverEffects(observer, [{ target: restoredPlayer, state: covMap[restoredPlayer.id] }]);
        }
      }
      ...
      expect(mockBatchUpdateVisibilityEffects).toHaveBeenCalledWith(existingNPC, [{ target: restoredPlayer, state: 'concealed' }]);
      expect(mockBatchUpdateCoverEffects).toHaveBeenCalledWith(existingNPC, [{ target: restoredPlayer, state: 'lesser' }]);
    });
  });
  ...
  describe('Race Condition Handling', () => {
    ...
    test('should handle parallel token deletion gracefully', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('Token cleanup: Invalid token reference during parallel deletion');
    });
  ...
  describe('Effect Management During Restoration', () => {
    ...
    test('should use correct PF2e condition icons for custom effects', () => {
      ...
      expect(getPF2eConditionIcon('undetected')).toBe('systems/pf2e/icons/conditions/undetected.webp');
      expect(getPF2eConditionIcon('nonexistent')).toBe('systems/pf2e/icons/conditions/nonexistent.webp');
    });
  });
