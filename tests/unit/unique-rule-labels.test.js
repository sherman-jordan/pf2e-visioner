...
describe('Cover Rule Label Uniqueness Test', () => {
    ...
    test('should create unique rule labels for each observer to prevent PF2e rule conflicts', async () => {
        ...
        const effects = mockTarget.actor.itemTypes.effect.filter(e =>
            e.flags?.['pf2e-visioner']?.aggregateCover === true
        );
        ...
        for (const effect of effects) {
            const acRules = effect.system.rules.filter(r =>
                r.key === 'FlatModifier' && r.selector === 'ac' && r.label
            );
        ...
        const duplicateLabels = ruleLabels.filter((label, index) =>
            ruleLabels.indexOf(label) !== index
        );
        ...
        if (ruleLabels.length > 1) {
            ...
            const areAllDifferent = ruleLabels.every((label, index) => 
                !ruleLabels.some((otherLabel, otherIndex) => 
                    index !== otherIndex && label === otherLabel
                )
            );
