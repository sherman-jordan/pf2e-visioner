...
describe('Visibility State Management Core Logic', () => {
  ...
  describe('updateSingleVisibilityEffect - Core State Management', () => {
    test('handles null/undefined tokens gracefully', async () => {
      ...
      await expect(updateSingleVisibilityEffect(null, validToken, 'hidden')).resolves.toBeUndefined();
      await expect(updateSingleVisibilityEffect(validToken, null, 'hidden')).resolves.toBeUndefined();
    ...
    test('handles corrupted effect data gracefully', async () => {
      ...
      await expect(updateSingleVisibilityEffect(observerToken, targetToken, 'hidden')).resolves.toBeUndefined();
    });
    ...
    test('processes visibility state transitions for valid creature actors', async () => {
      ...
      const totalCalls = actor.createEmbeddedDocuments.mock.calls.length + 
    ...
    test('handles different effect target directions', async () => {
      ...
      const totalObserverCalls = observerActor.createEmbeddedDocuments.mock.calls.length + 
  ...
  describe('Visibility Effect Edge Cases', () => {
    ...
    test('handles various visibility state values', async () => {
      ...
      for (const state of states) {
        await expect(updateSingleVisibilityEffect(observerToken, targetToken, state)).resolves.toBeUndefined();
      }

      // Test invalid states
      await expect(updateSingleVisibilityEffect(observerToken, targetToken, 'invalid')).resolves.toBeUndefined();
      await expect(updateSingleVisibilityEffect(observerToken, targetToken, null)).resolves.toBeUndefined();
    });
    ...
    test('handles removeAllEffects option', async () => {
      ...
      await expect(updateSingleVisibilityEffect(observerToken, targetToken, 'hidden', {
